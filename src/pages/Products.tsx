...
const Products = () => {
  ...
  const { data: products, isLoading } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("products")
        .select("id,name,name_bn,price,compare_price,image_url,weight,grade,coming_soon,serial_number,is_active,category_id,vendor_id,created_at,categories(name,name_bn)")
        .eq("is_active", true);
      if (selectedCategory) {
